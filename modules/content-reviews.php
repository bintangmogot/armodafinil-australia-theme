...
<section class="py-14 lg:py-20 px-6 lg:px-12 bg-white">
    <div class="max-w-5xl mx-auto">
        <?php if ($heading) : ?>
            <h2 class="text-2xl lg:text-3xl font-bold text-center text-[#0a1045] mb-10"><?php echo esc_html($heading); ?></h2>
        <?php endif; ?>
        <?php if ($reviews->have_posts()) : ?>
            <div class="grid md:grid-cols-2 gap-6">
                <?php while ($reviews->have_posts()) : $reviews->the_post(); 
                    ...
                    $name   = get_field('name');
                ?>
                    <div class="bg-[#f5f7fb] rounded-2xl p-6 border border-[#0a1045]/5 hover:shadow-md transition-shadow">
                        <?php if ($rating) : ?>
